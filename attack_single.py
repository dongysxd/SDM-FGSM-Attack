...
tf.flags.DEFINE_string('model_type', 'vgg_19', 'vgg_19/inception_v4/res_v2_152/inc_res_v2')
tf.flags.DEFINE_integer('noise', 32, 'l_inf size of noise')
tf.flags.DEFINE_string('csv_file', './datasets/data.csv', 'csv_file path')
tf.flags.DEFINE_string('input_dir', './datasets/images', 'dev_data path')
tf.flags.DEFINE_string('output_dir', './results', 'output_file path ')
...
if FLAGS.output_dir == './results':
    FLAGS.output_dir = './results/%s' % FLAGS.model_type
...
def perspective_transformation(w, h, angle_x, angle_y):
    """
    ...
    for x_hat in xs:
        ...
        if model == 'vgg_19':
            ...
            with slim.arg_scope(inception.inception_v4_arg_scope()):
                _, end_points_inc_v4 = inception.inception_v4(x_inc, reuse=tf.AUTO_REUSE, num_classes=1001,
                                                              is_training=False,
                                                              scope='InceptionV4')
            ...
            with slim.arg_scope(resnet_v2.resnet_arg_scope()):
                ...
                end_points_resnet_v2_152['logits'] = tf.squeeze(end_points_resnet_v2_152['resnet_v2_152/logits'],
                                                                [1, 2])
            logit_resnet = end_points_resnet_v2_152['logits']
            loss_resnet = tf.losses.softmax_cross_entropy(one_hot_1001, logit_resnet / 5, label_smoothing=0.0,
                                                          weights=1.0)
            ...
            loss_inc_res = tf.losses.softmax_cross_entropy(one_hot_1001, logit_inc_res / 5, label_smoothing=0.0,
                                                           weights=1.0)
            ...
            raise NotImplementedError('Please choose one model_type from vgg_19/inception_v4/res_v2_152/inc_res_v2.')
...
def process_transformation(anglex, angley, imgs, saliency_maps, xmin, xmax):
    """
    ...
    warp = perspective_transformation(FLAGS.image_width, FLAGS.image_height, anglex, angley)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        gaussian = get_gaussian_kernel(radius=3, num_of_models=1)
        ...
        # load models
        if model == 'vgg_19':
            s = tf.train.Saver(slim.get_model_variables(scope='vgg_19'))
        elif model == 'inception_v4':
            s = tf.train.Saver(slim.get_model_variables(scope='InceptionV4'))
        elif model == 'res_v2_152':
            s = tf.train.Saver(slim.get_model_variables(scope='resnet_v2_152'))
        elif model == 'inc_res_v2':
            s = tf.train.Saver(slim.get_model_variables(scope='InceptionResnetV2'))
        else:
            raise NotImplementedError('Please choose one model_type from vgg_19/inception_v4/res_v2_152/inc_res_v2.')
        ...
        with tf.compat.v1.Session(config=config) as sess:
            ...
            if model == 'vgg_19':
                s.restore(sess, model_checkpoint_map['vgg_19'])
            elif model == 'inception_v4':
                s.restore(sess, model_checkpoint_map['inception_v4'])
            elif model == 'res_v2_152':
                s.restore(sess, model_checkpoint_map['resnet_v2_152'])
            elif model == 'inc_res_v2':
                s.restore(sess, model_checkpoint_map['inception_resnet_v2'])
            else:
                raise NotImplementedError(
                    'Please choose one model_type from vgg_19/inception_v4/res_v2_152/inc_res_v2.')
            ...
            for filenames, raw_images, target_labels_1000, target_labels_1001, s_maps in \
                ...
                for i in range(min_angle, max_angle):
                    ...
                    original_x_angle, s_maps_angle, x_min_angle, x_max_angle = process_transformation(anglex, angley,
                                                                                                       original_x,
                                                                                                       s_maps,
                                                                                                       xmax=x_max,
                                                                                                       xmin=x_min)
                    tau = (eps / (num_angles / 2)) * step
